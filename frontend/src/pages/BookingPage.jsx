...
import { getVendorById, getUserById } from '../api.js'; // Import the API to fetch vendor and user data
...
const BookingPage = () => {
    const { id } = useParams(); // Extract vendorId from URL
    ...
    const [vendorUserInfo, setVendorUserInfo] = useState(null); // State to hold the user info (name, email)

    useEffect(() => {
        const fetchVendorData = async () => {
            try {
                // Step 1: Fetch the vendor data
                const vendorResponse = await getVendorById(id);
                const vendorData = vendorResponse.data;
                setVendor(vendorData);

                // Step 2: Extract userId from vendor and fetch the user info
                const vendorUserId = vendorData.userId; // Assuming userId is stored in vendor data
                const userResponse = await getUserById(vendorUserId); // Fetch user by userId
                setVendorUserInfo(userResponse.data); // Set user info (name, email, etc.)
            ...
            <div className="max-w-4xl w-full mx-auto bg-white shadow-xl rounded-lg p-8 z-10">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between mb-8">
                    <div className="md:w-1/2 flex flex-col items-center md:items-start mb-4 md:mb-0">
                        ...
                        {/* Display vendor user's name, occupation, and email */}
                        <h2 className="text-4xl font-extrabold text-gray-900 mb-2">
                            {vendorUserInfo?.name || "Name not available"}
                        </h2>
                        <p className="text-lg text-gray-700">
                            {vendor.occupation || "Occupation not available"} {/* Add occupation here */}
                        </p>
